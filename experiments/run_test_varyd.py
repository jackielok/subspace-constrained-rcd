...
n_iter_cd = 1500           # number of CD iterations
n_iter_cg = 100            # number of CG iterations
rel_rnorm_tol = -1         # terminate if relative residual norm falls below threshold
save_data = True

d_list = [0, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000]    # list of approximation ranks for RPCholesky to run over
...
def run_test_varyd(
    ...
    # PCG
    K_pcg, K_pcg_plotdata = test_helper.run_pcg(K + D, ytrain, F=F2, lamb=lamb, x_init=K_scrcd.x_init, n_iter=n_iter_cg, rel_rnorm_tol=rel_rnorm_tol)
