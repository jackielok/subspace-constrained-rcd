#!/usr/bin/env python3
### Plots averaged outputs of run_test_varyl.py

import numpy as np
from matplotlib import pyplot as plt
import plot_helper
import run_test_varyl
import argparse
...
solver_id_list = ["RCD", "SCRCD", "SCRCD2"]
color_list = ["tab:green", "tab:blue", "blue"]
label_list = ["RCD", "SCRCD (diag)", "SCRCD (unif)"]
linestyle_list = [(5, (10, 3)), "solid", (0, (6, 1, 1, 1))]
linewidth_list = [1.75, 1.75, 1.75]

#################### Define functions ####################

def plot_run_test_varyl(
    dataset_id = run_test_varyl.selected_dataset,
    out_loc = run_test_varyl.out_loc,
    l_list = run_test_varyl.l_list,
    d = run_test_varyl.d,
    ...
    figs_loc = figs_loc,
    n_samples = run_test_varyl.n_samples,
...
if __name__ == "__main__":
    ...
    # Optional arguments
    parser.add_argument("--selected_dataset", type=str, default=run_test_varyl.selected_dataset, help="Selected dataset")
    parser.add_argument("--l_list", type=int, nargs="+", default=run_test_varyl.l_list, help="List of block sizes for coordinate descent (space-separated)")
    parser.add_argument("--d", type=int, default=run_test_varyl.d, help="Approximation rank for RPCholesky")
    parser.add_argument("--n_samples", type=int, default=run_test_varyl.n_samples, help="Number of samples to average over")
