...
def cholesky_helper(A, k, alg, stoptol = 0, rng = None):
    if not isinstance(A, AbstractPSDMatrix):
        A = PSDMatrix(A)
    
    if isinstance(A, PSDMatrix):
        sparse_A = sp.sparse.issparse(A.matrix)
    else:
        sparse_A = False
    ...
    for i in range(k):
        ...
        arr_idx.append(idx)
        rows[i,:] = A[idx,:] if not sparse_A else A[idx,:].toarray()
...
def block_cholesky_helper(A, k, b, alg, stoptol = 1e-14, strategy = "regularize", rbrp_atol = 0.0, rbrp_rtol = "1/b", rng = None):
    if not isinstance(A, AbstractPSDMatrix):
        A = PSDMatrix(A)
    ...
    while counter < k:
        ...
        if "regularize" == strategy or "regularized" == strategy:
            arr_idx.extend(idx)
            rows[counter:counter+block_size,:] = A[idx,:] if not sparse_A else A[idx,:].toarray()
            ...
            if not sparse_A:
                ...
                H = A[idx,idx].toarray() - G[0:counter,idx].T @ G[0:counter,idx]
            L, piv, rank = _greedy_cholesky(H, rtol = rbrp_rtol, atol = rbrp_atol)
            idx = idx[piv[0:rank]-1]
            arr_idx.extend(idx)
            rows[counter:counter+len(idx),:] = A[idx,:] if not sparse_A else A[idx,:].toarray()
...
def rejection_cholesky(H, rng = None):
    if rng is None:
        rng = np.random.default_rng()
...
def accelerated_rpcholesky(A, k, b = "auto", stoptol = 1e-13, verbose = False, rng = None):
    if rng is None:
        rng = np.random.default_rng()
    ...
    while counter < k:
        ...
        H = np.array(A[idx, idx] - G[0:counter,idx].T @ G[0:counter,idx])
        L, accepted = rejection_cholesky(H, rng=rng)
        ...
        rows[counter:counter+num_sel,:] = A[idx,:] if not sparse_A else A[idx,:].toarray()
...
def block_greedy(A, k, b = 100, stoptol = None, **kwargs):
    return block_cholesky_helper(A, k, b, 'greedy', **kwargs)
