...
def LaplaceKernel(x,y,bandwidth=1.0,**kwargs):
    ...
    return np.exp( -np.sum(np.abs(x - y)) / bandwidth )
    # return np.exp( -np.linalg.norm(x-y, ord=1) / bandwidth )  # wrong (matrix) norm when x, y are given as row vectors

def LaplaceKernel_vec(vec_x,vec_y,bandwidth=1.0,**kwargs):
    # for vec_x, vec_y in R^{n*d}, return n values
    if sp.sparse.issparse(vec_x) or sp.sparse.issparse(vec_y):
        dsts = sp.sparse.linalg.norm(vec_x-vec_y, ord = 1, axis = -1)
    else:   
...
def GaussianKernel(x,y,bandwidth=1.0,**kwargs):
    # for single x,y in R^d
    d = x - y
    return np.exp(-0.5*np.dot(d,d)/bandwidth**2)
